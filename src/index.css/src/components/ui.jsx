                          ...
                          export const darkHdr = {
                            background:'#0e1a0e', padding:'24px 22px 40px',
                              ...
                              export function Spinner() {
                                return <div style={{ width:16, height:16, border:'2px solid rgba(255,255,255,.3)', borderTopColor:'#fff', borderRadius:'50%', animation:'spin .7s linear infinite', flexShrink:0 }} />
                                }

                                export function Toast({ msg }) {
                                  if (!msg) return null
                                    return <div style={{ position:'fixed', bottom:24, left:'50%', transform:'translateX(-50%)', background:'#0e1a0e', color:'#fff', borderRadius:10, padding:'13px 22px', fontSize:13.5, fontWeight:500, boxShadow:'0 8px 32px rgba(0,0,0,.22)', zIndex:9999, whiteSpace:'nowrap' }}>{msg}</div>
                                    }
                                                                                                        ...
                                                                                                        >
                                                                                                              {children}
                                                                                                                    ...
                                                                                                                    export function SectionLabel({ children }) {
                                                                                                                      return <div style={{ fontSize:10.5, fontWeight:700, letterSpacing:1.8, textTransform:'uppercase', color:'rgba(14,26,14,0.5)', marginBottom:10 }}>{children}</div>
                                                                                                                      }

                                                                                                                      export function BackBtn({ onClick }) {
                                                                                                                        return <button onClick={onClick} style={{ background:'none', border:'none', color:'rgba(255,255,255,.5)', fontFamily:'inherit', fontSize:13, cursor:'pointer', padding:0, marginBottom:18 }}>← Back</button>
                                                                                                                        }
}